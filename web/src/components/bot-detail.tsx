...
import { api, type BotCmd } from '@/lib/api'
...
// === TAMBAHKAN KODE INI DI BAWAH IMPORTS ===
const debugC2 = (action: string, targets: number[], data?: any) => {
  console.log(
    `%c[C2 DEBUG]%c 🎯 Action: %c${action}%c | 🤖 Targets (${targets.length}): %c[${targets.join(", ")}]`,
    "color: #fff; background: #8b5cf6; padding: 2px 4px; border-radius: 4px; font-weight: bold;",
    "color: inherit;",
...
const STATUS_DOT: Record<string, string> = {
  "Offline": "bg-red-500",            // Merah
  "Connecting": "bg-yellow-500",      // Kuning
  "Connected": "bg-blue-500",         // Biru
  "In Game": "bg-green-500",          // Hijau (bisa juga pakai bg-emerald-500)
  "Two Factor Auth": "bg-orange-500",
  "Server Overloaded": "bg-red-500",
  "Too Many Logins": "bg-purple-500",
  "Update Required": "bg-gray-500",
...
export function BotDetail({ bot }: { bot: LiveBot }) {
  ...
  const selectedBots = useAtomValue(selectedBotsAtom);

  async function sendToSelectedBots(cmd: BotCmd) {
    const targets =
      selectedBots.size > 0
      ...
      <div className="shrink-0 flex items-center gap-2 px-4 h-10 bg-card border-b border-border">
        <span
          className={cn(
            ...
            STATUS_DOT[bot.status] || "bg-gray-500"
          )}
        />
        ...
        <button
          title="Reconnect"
          onClick={() => sendToSelectedBots({ type: "reconnect" })}
        ...
        <button
          title="Disconnect"
          onClick={() => sendToSelectedBots({ type: "disconnect" })}
          className="w-5 h-5 flex items-center justify-center rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"
        >
...
function WarpInput({ botId }: { botId: number }) {
  ...
  const targets =
    selectedBots.size > 0
      ...
      debugC2("WARP", targets, { world_name: name, door_id: doorId.trim() });
      ...
      <div className="flex flex-col gap-4">
        <Section label="Account">
          ...
          <button
            ...
            onClick={() => {
              const targets = selectedBots.size > 0 ? Array.from(selectedBots) : [bot.id];
               debugC2("ACCEPT_ACCESS", targets);
              targets.forEach((id) => {
                api.sendCmd(id, { type: "accept_access" }).catch(() => {});
              });
            }}
          >
            Accept Access
...
function AutoCollectRangePanel({
  ...
  async function sendCollectConfig(nextRadius: number, nextBlacklist: number[]) {
    // 1. Tentukan target (satu atau banyak)
    const targets = selectedBots.size > 0 ? Array.from(selectedBots) : [botId];
    
    setCollectRadius(nextRadius)
    setBlacklist(nextBlacklist)

    // 2. Update UI (Jotai State) untuk SEMUA bot yang dicentang
    setBots((m) => {
    ...
    // 3. Tembak command ke backend Rust
    targets.forEach((id) => {
      api.sendCmd(id, {
...
function ScriptTab({ botId }: { botId: number }) {
  ...
  selectedBots.size > 0
    ? Array.from(selectedBots)
    : [botId];
debugC2("RUN_SCRIPT", targets, { script_length: script.length });
await Promise.all(
  targets.map((id) =>
    api.sendCmd(id, {
      type: "run_script",
...
function ConfigTab({
  ...
  async function save() {
    ...
    const targets = selectedBots.size > 0 ? Array.from(selectedBots) : [botId];
    debugC2("SET_DELAYS", targets, newDelays);
    try {
      // 1. Tembak ke semua bot yang dicentang
      targets.forEach((id) => {
        api.sendCmd(id, { type: "set_delays", ...newDelays }).catch(() => {});
      });

      // 2. Update UI Jotai untuk semua bot yang dicentang
      setBots((m) => {
        const nextMap = new Map(m);
        targets.forEach((id) => {
          const bot = nextMap.get(id);
          if (bot) nextMap.set(id, { ...bot, delays: newDelays });
        });
        return nextMap;
      });
  ...
  // FUNGSI BARU UNTUK TOGGLE SWITCH MASSAL
  const toggleBehavior = (type: 'set_auto_collect' | 'set_auto_reconnect', enabled: boolean) => {
    const targets = selectedBots.size > 0 ? Array.from(selectedBots) : [botId];
    debugC2(type.toUpperCase(), targets, { enabled });
    // Tembak command ke backend
    targets.forEach((id) => {
      api.sendCmd(id, { type, enabled }).catch(() => {});
    });
    ...
    setBots((m) => {
      ...
      targets.forEach((id) => {
        const bot = nextMap.get(id);
        if (bot) {
          if (type === 'set_auto_collect') nextMap.set(id, { ...bot, auto_collect: enabled });
          if (type === 'set_auto_reconnect') nextMap.set(id, { ...bot, auto_reconnect: enabled });
        }
      });
      return nextMap;
    });
  ...
  return (
    <div className="max-w-xs flex flex-col gap-4">
      ...
      {/* GANTI KEDUA SWITCH MENJADI SEPERTI INI */}
      <label className="flex items-center gap-2 cursor-pointer">
        <Switch
          size="sm"
          checked={autoCollect}
          onCheckedChange={(enabled) => toggleBehavior('set_auto_collect', enabled)}
        />
        <span className="text-xs text-muted-foreground">
      ...
      <label className="flex items-center gap-2 cursor-pointer">
        <Switch
          ...
          onCheckedChange={(enabled) => toggleBehavior('set_auto_reconnect', enabled)}
        />
        <span className="text-xs text-muted-foreground">
...
function InventoryTable({
  ...
  async function cmd(type: string, item_id: number, count?: number) {
    const targets = selectedBots.size > 0 ? Array.from(selectedBots) : [botId];
    if (type === "wear" || type === "unwear") {
  setBots((m) => {
    const next = new Map(m);

    targets.forEach((id) => {
      const bot = next.get(id);
      if (!bot) return;

      next.set(id, {
        ...bot,
        inventory: bot.inventory.map((item) => ({
          ...item,
          is_active:
    ...
    debugC2(type.toUpperCase(), targets, { item_id, count });

    targets.forEach(async (id) => {
      // Auto-Chunking: Jika drop/trash lebih dari 200, potong otomatis
      if ((type === "drop" || type === "trash") && count !== undefined && count > 200) {
        let remaining = count;
        while (remaining > 0) {
          const currentChunk = Math.min(remaining, 200);
          
          await api.sendCmd(id, {
            type,
            item_id,
            count: currentChunk,
          } as never).catch(() => {});
          ...
          // Jeda 400ms agar server tidak kick/DC bot
          if (remaining > 0) {
      ...
      <Table className="text-xs">
        <TableHeader className="sticky top-0 bg-card">
          <TableRow className="border-b border-border">
            ...
            <TableHead>Actions {selectedBots.size > 0 ? '(Mass)' : ''}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {inventory.length === 0 ? (
            ...
            [...inventory]
              ...
              .map((i) => {
                const inputVal = customAmounts[i.item_id] ?? "";
                const actionAmount = parseInt(inputVal) || 1;
                  ...
                  <TableRow key={i.item_id}>
                    ...
                    <TableCell className="px-2 py-1">
                      <div className="flex gap-1 flex-wrap items-center">
                        ...
                        {/* VALIDASI INPUT LANGSUNG (LIVE VALIDATION) */}
                        <Input
                          type="number"
                          min={1}
                          max={i.amount}
                          ...
                          onChange={(e) => {
                            ...
                            if (val !== "") {
                              ...
                              if (isNaN(num)) val = "";
                              else if (num > i.amount) val = String(i.amount); // Kunci di batas maksimum
                              else if (num < 1) val = "1";                     // Jika 0 atau minus, ubah ke 1
                              else val = String(num);
                            }
                            setCustomAmounts((prev) => ({ ...prev, [i.item_id]: val }));
                          }}
                          className="h-6 w-14 text-[11px] px-1 py-0 text-center"
                        />

                        <Btn onClick={() => cmd("drop", i.item_id, actionAmount)}>Drop</Btn>
                        <Btn onClick={() => cmd("drop", i.item_id, i.amount)}>
                          Drop All
                        </Btn>
                        <Btn
                          onClick={() => cmd("trash", i.item_id, actionAmount)}
                          variant="danger"
                        >
                          Trash
                        </Btn>
  ...
  const move = (x: number, y: number) => {
    const targets =
      selectedBots.size > 0
        ? Array.from(selectedBots)
        : [botId];
//console.log("WalkTo Targets:", targets)
debugC2("MOVE", targets, { x, y });
    targets.forEach((id) => {
      api.sendCmd(id, {
        type: "move",
        x,
