...
import { botsAtom, selectedBotIdAtom, selectedBotsAtom} from '@/lib/store'
...
const STATUS_LABEL: Record<string, string> = {
  ...
  "Too Many Logins": 'Too Many Logins',
...
function BotList() {
  ...
  const [selectedId, setSelectedId] =
    useAtom(selectedBotIdAtom)

  const [selectedBots, setSelectedBots] =
    useAtom(selectedBotsAtom)
...
return (
  <>
    <div className="px-2 py-1 text-[10px] text-muted-foreground border-b">
      Selected: {selectedCount} bot{selectedCount !== 1 ? "s" : ""}
    </div>
    ...
    <div className="p-1.5 space-y-1">
      {[...bots.values()].map((bot) => (
        <BotItem
          ...
          onToggle={() => {
            const next = new Set<number>(selectedBots)
