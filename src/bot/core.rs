...
rid|{}\nplatformID|0,1,1\ndeviceVersion|0\ncountry|jp\nhash|{}\nmac|{}\nwk|{}\nzf|31631978\nlmode|1\n",
        ...
        while let Some(event) = self.host.next_event() {
            match event {
                ...
                enet::EventNoRef::Disconnect { peer: id, .. } => {
                    ...
                    // --- LOGIKA STATUS YANG SINKRON DENGAN fmt::Display ---
                    if let Some(r) = self.redirect.as_ref() {
                        { self.state.write().unwrap().status = BotStatus::Connecting; }
                        self.emit(WsEvent::BotStatus { bot_id: self.bot_id, status: BotStatus::Connecting.to_string() });
                        ...
                        // Sedang menunggu cooldown (Status sudah diset sebelumnya, misal 2FA/Maintenance)
                        
                    } else if self.manual_disconnect {
                        self.manual_disconnect = false;
                        self.log_console("[Bot] Disconnected manually. Auto-reconnect is ignored.".to_string());
                        { self.state.write().unwrap().status = BotStatus::Offline; }
                        self.emit(WsEvent::BotStatus { bot_id: self.bot_id, status: BotStatus::Offline.to_string() });
                        ...
                        if self.reconnect_interval > 0 {
                            self.log_console(format!("[Bot] Server disconnected — reconnecting in {}ms", self.reconnect_interval));
                            self.reconnect_after = Some(std::time::Instant::now() + std::time::Duration::from_millis(self.reconnect_interval));
                        } else {
                            self.log_console("[Bot] Server disconnected — re-fetching token and server data".to_string());
                            self.reconnect_main();
                        }
                    } else {
                        { self.state.write().unwrap().status = BotStatus::Offline; }
                        self.emit(WsEvent::BotStatus { bot_id: self.bot_id, status: BotStatus::Offline.to_string() });
                        self.log_console("[Bot] Server disconnected — auto-reconnect is disabled".to_string());
                    }
                }
                            ...
                            if s.contains("action|logon_fail") {
                                if self.pending_2fa {
                                    ...
                                    self.emit(WsEvent::BotStatus {
                                        bot_id: self.bot_id,
                                        status: BotStatus::TwoFactorAuth.to_string()
                                    });
                                    ...
                                    self.emit(WsEvent::BotStatus {
                                        bot_id: self.bot_id,
                                        status: BotStatus::ServerOverloaded.to_string()
                                    });
                                    ...
                                    self.emit(WsEvent::BotStatus {
                                        bot_id: self.bot_id,
                                        status: BotStatus::TooManyLogins.to_string()
                                    });
                                    ...
                                    self.emit(WsEvent::BotStatus {
                                        bot_id: self.bot_id,
                                        status: BotStatus::UpdateRequired.to_string()
                                    });
                                    self.stop_requested = true;
                                    ...
                                    self.emit(WsEvent::BotStatus {
                                        bot_id: self.bot_id,
                                        status: BotStatus::Maintenance.to_string()
                                    });
                                } else {
                                    self.log_console(
                                ...
                                GamePacketType::SendMapData => {
                                    ...
                                    match World::parse(&pkt.extra_data) {
                                        Ok(world) => {
                                            ...
                                            self.emit(WsEvent::BotStatus {
                                                bot_id: self.bot_id,
                                                status: BotStatus::InGame.to_string()
                                            });
                                            self.emit(WsEvent::BotWorld {
                                                bot_id: self.bot_id,
    ...
    fn on_call_function(&mut self, id: enet::PeerID, extra_data: &[u8]) {
        ...
        match fn_name.as_str() {
            ...
            "OnSuperMainStartAcceptLogonHrdxs47254722215a" => {
                ...
                self.emit(WsEvent::BotStatus {
                    bot_id: self.bot_id,
                    status: BotStatus::Connected.to_string()
                });
            }
            "OnRemove" => {
                let message = vl.get(1).map(|v| v.as_string()).unwrap_or_default();
    ...
    pub fn wear(&mut self, item_id: u32) {
        ...
        // Cari informasi item yang baru saja dipakai
        if let Some(target_info) = self.items_dat.find_by_id(item_id) {
            
            // HANYA CEK clothing_type != 0 (Abaikan action_type agar Pickaxe & alat lain terbaca)
            if target_info.clothing_type != 0 {
                let mut conflicts = Vec::new();

                for (id, inv_item) in self.inventory.items.iter() {
                    // skip item yang sedang tidak dipakai
                    if inv_item.flag & 1 == 0 {
                        continue;
                    }

                    // skip item yang sama dengan yang baru diklik
                    if *id == item_id as u16 {
                        continue;
                    }

                    // Cek barang lama di database
                    if let Some(info) = self.items_dat.find_by_id(*id as u32) {
                        // Jika slot pakainya (clothing_type) SAMA, berarti bentrok di 1 tempat (misal: sama-sama di tangan)
                        if info.clothing_type == target_info.clothing_type {
                            conflicts.push(*id);
                        }
                    }
                }

                // Matikan status barang lama (Auto-Unwear)
                for id in conflicts {
                    if let Some(item) = self.inventory.items.get_mut(&id) {
                        item.flag &= !1; // Mematikan bit 1
                    }
                }
            }
        }

        // Nyalakan status barang baru yang diklik
        if let Some(item) = self.inventory.items.get_mut(&(item_id as u16)) {
            item.flag |= 1; // Menyalakan bit 1
        }
    ...
    fn handle_command(&mut self, cmd: BotCommand) {
        match cmd {
            ...
            BotCommand::Reconnect => {
                // Pastikan tanda manual dimatikan agar bot bisa hidup lagi
                self.manual_disconnect = false;
                {
                    self.state.write().unwrap().status = BotStatus::Connecting;
                }
                ...
                if self.peer_id.is_some() {
                    self.disconnect();
                    // Paksa bot untuk reconnect instan setelah terputus
                    self.reconnect_after = Some(std::time::Instant::now() + std::time::Duration::from_millis(50));
                } else {
                    self.reconnect_main();
                }
            }
